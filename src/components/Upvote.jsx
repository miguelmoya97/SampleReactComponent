...
const Upvote = (props) => {
    const { selected, identifier, changeSelection } = props
    return <>
        <div className={`${styles.upvoteContainer} ${selected ? styles.selected : ''}`} onClick={changeSelection} data-testid={`upvote-${identifier}`}>
            <svg className={`${styles.arrow} ${selected ? styles.selected : ''}`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
